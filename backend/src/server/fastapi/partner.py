...
from backend.src.services.partner import get_foreign_profile, get_likes_service
...
@router.get("/likes")
async def get_likes_router(token: Annotated[dict, Depends(verify_token)]):
    return await get_likes_service(token["id"])
